...
Main package for the project. Everything lives here — algorithms, environments, adaptation logic, and scripts.
...
- [`policy.py`](algorithms/policy.py) — Actor-critic network. Two-headed MLP (shared backbone → policy head + value head). MC dropout is always-on so we can sample variance during deployment.
...
- [`formation_aviary.py`](envs/formation_aviary.py) — `FormationAviary` environment. Two drones learn to fly in formation at a target height with a target spacing. Reward penalizes distance from formation targets + height error + excessive tilting. Extends `BaseAviary` from gym-pybullet-drones.
...
- [`logger.py`](utils/logger.py) — Dual logger: writes TensorBoard events + CSV files. Used during both training and evaluation.
